        ...
        private void Update()
        {
            if (spaceVehicleEngines != null && spaceVehicleEngines.EnginesActivated)
            {
                if (spaceEnginesCruisingAudio != null)
                {
                    ...
                    float engineVolumeAmount = lateralThrustContribution * Mathf.Abs(spaceVehicleEngines.TranslationThrottleValues.x) +
                                        verticalThrustContribution * Mathf.Abs(spaceVehicleEngines.TranslationThrottleValues.y) +
                                        forwardThrustContribution * Mathf.Abs(spaceVehicleEngines.TranslationThrottleValues.z);
                ...
                {
                    float boostAmount = Mathf.Clamp(spaceVehicleEngines.BoostThrottleValues.magnitude, 0f, 1f);
