...
namespace VSX.UniversalVehicleCombat
{
    ...
    public class SceneOriginManager : MonoBehaviour
    {
        ...
        void ShiftScene()
        {
            ...
            // Move the scene
            Vector3 playerRelativePosition = GameAgentManager.Instance.FocusedGameAgent.Vehicle.transform.position - transform.position;
            transform.position = -playerRelativePosition;


            // Store the relative camera position
            Vector3 cameraOffset = vehicleCamera.transform.position - GameAgentManager.Instance.FocusedGameAgent.Vehicle.transform.position;

            // Put the player in the center
            GameAgentManager.Instance.FocusedGameAgent.Vehicle.transform.position = Vector3.zero;

            vehicleCamera.transform.position = GameAgentManager.Instance.FocusedGameAgent.Vehicle.transform.position + cameraOffset;
        ...
        void Update()
        {
            ...
            Vector3 playerRealPos = GameAgentManager.Instance.FocusedGameAgent.Vehicle.transform.position;
